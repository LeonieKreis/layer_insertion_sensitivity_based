    ...
    if True:
        with open(f"results_data_spirals/Exp{k}_1.json") as file: #abs max
            ...
            for i in range(len(a_temp)):
                if len(a_temp[i])!=500: print('incorrect lentgh at',i)
                a_temp[i]=a_temp[i]+list(np.zeros(500-len(a_temp[i])))
        ...
        with open(f"results_data_spirals/Exp{k}_4.json") as file: # big resnet
            ...
            if plot_error:
                ...
                for i in range(10):
                    if len(a_err[i])!=500: print('incorrect lentgh at',i)
            if plot_grads:
                a_grad = f[run]['grad_norms']
            a4 = np.array(a_temp)
            if plot_error:
        ...
        labels = ['LI','LIother', 'ResNet1','ResNet2','random']

        methods = (a,a2,a3,a4,a5)
        times = (at,at2,at3,at4,at5)
        ...
        for i, (aa, ta) in enumerate(zip(methods, times)):
            ...
            plt.vlines(100*1, 0, 1, colors='gray', linestyles='dashed')
            plt.vlines(200*1, 0, 1, colors='gray', linestyles='dashed')
            plt.vlines(300*1, 0, 1, colors='gray', linestyles='dashed')
            plt.legend()
            #plt.ylim([0, 2])
        ...
        if plot_error:
            ...
            for i, (aa, ta) in enumerate(zip(methods_e, times)):
                ...
                plt.vlines(200, 0, 100, colors='gray', linestyles='dashed')
                plt.vlines(300, 0, 100, colors='gray', linestyles='dashed')
                plt.legend()
                plt.ylim([0, 100])
                #ma = max([a.shape[1] for a in methods])
                # plt.xlim([0, ma])
