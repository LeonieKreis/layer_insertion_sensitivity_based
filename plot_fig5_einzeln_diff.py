...
for i in range(14,15):
    ...
    diff = [mean2[i]-mean1[i] for i in range(len(mean1))]
    plt.plot(list(range(len(mean1))),diff, label = 'FNN1-LI', linewidth=3)
    print(diff)
    ...
    plt.tight_layout()
    plt.savefig('tikzpicture_plots/fig4_diff.pdf', format="pdf", bbox_inches="tight")
    plt.show()