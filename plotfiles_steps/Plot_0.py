...
def plot_loss_error_single_init(plot_error, plot_grads, log_scale, run,k, save = False):
    ...
    if True:
        ...
        for i, (aa, ta) in enumerate(zip(methods, times)):
            ...
            plt.vlines(500*8, 0, 1, colors='gray', linestyles='dashed')
            plt.vlines(2*500*8, 0, 1, colors='gray', linestyles='dashed')
            plt.vlines(3*500*8, 0, 1, colors='gray', linestyles='dashed')
            plt.vlines(4*500*8, 0, 1, colors='gray', linestyles='dashed')
            plt.vlines(5*500*8, 0, 1, colors='gray', linestyles='dashed')
            plt.legend()
            #plt.ylim([0, 2])
