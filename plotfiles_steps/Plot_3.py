    ...
    if True:
        ...
        if plot_error:
            ...
            for i, (aa, ta) in enumerate(zip(methods_e, times)):
                ...
                plt.plot( mean1, colors_[i] , label=label,
                            markersize=5)  # , linestyle='o')
                plt.vlines(500, 0, 10, colors='gray', linestyles='dashed')
                plt.vlines(2*500, 0, 10, colors='gray', linestyles='dashed')
                plt.vlines(3*500, 0, 10, colors='gray', linestyles='dashed')
                plt.vlines(4*500, 0, 10, colors='gray', linestyles='dashed')
                plt.vlines(5*500, 0, 10, colors='gray', linestyles='dashed')
                plt.legend()
                plt.yscale('log')
                #ma = max([a.shape[1] for a in methods])
                # plt.xlim([0, ma])
