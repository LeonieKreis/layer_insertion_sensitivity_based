...
for i in range(no_of_initializations):
    ...
    param_init1 = copy.deepcopy(param_init.data)
    param_init2 = copy.deepcopy(param_init.data)
    param_init_class = copy.deepcopy(param_init.data)

    torch.nn.utils.vector_to_parameters(param_init1, model_init.parameters())
    ...
    if _type == 'fwd':
        model_init2 = feed_forward(dim_in, dim_out, **kwargs_net2)
    if _type == 'res2':
        model_init2 = two_weight_resnet(dim_in, dim_out, **kwargs_net2)
    if _type == 'res1':
        model_init2 = one_weight_resnet(dim_in, dim_out, **kwargs_net2)
