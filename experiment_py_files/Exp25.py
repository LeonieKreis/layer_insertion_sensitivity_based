...
save_heatmapslimax = False
save_heatmaps_baseline_after = False
save_heatmaps=False
...
path0 = f'results_data_spirals/Exp{k}_0.json'
...
for i in range(no_of_initializations):
    ...
    if T0:
        ...
        final_params = torch.nn.utils.parameters_to_vector(model_init.parameters()).detach().numpy().tolist()
        write_losses(path0,
                    mblosses_classical1, max_length, end_list, test_error_classical1,
                    interval_testerror=interval_testerror, times=times31,grad_norms = grad_norm31, its_per_epoch=no_steps_per_epoch,
    ...
    if T2:
        ...
        if _type == 'fwd':
            model_init2 = feed_forward(dim_in, dim_out, **kwargs_net_classical)
        if _type == 'res2':
            model_init2 = two_weight_resnet(
                dim_in, dim_out, **kwargs_net_classical)
        if _type == 'res1':
            model_init2 = one_weight_resnet(
                dim_in, dim_out, **kwargs_net_classical)
        ...
        if plot:
            ...
            plt.show()
        # save losses2
        write_losses(f'results_data_spirals/Exp{k}_2.json', mb_losses2, max_length, end_list, test_errors2,
                   interval_testerror=interval_testerror, times=times2, grad_norms = grad_norm2, its_per_epoch=no_steps_per_epoch,
                   final_params=final_params, sens=sens2)
        # full_list_of_losses_2.append(mb_losses_comp)
