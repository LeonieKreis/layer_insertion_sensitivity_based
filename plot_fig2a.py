...
if plot_b:
    with open(f"results_data/Exp8_3.json") as file: #Exp8_3 or k_4
...
mean1.pop(line)
print(len(mean1))
axes[0].plot(mean1, '-', label=labels[0], linewidth= 3)

mean2 = np.nanmean(b, axis=0)
axes[0].plot(mean2, ':', label=labels[1], linewidth= 3)

mean3 = np.nanmean(c, axis=0)
axes[0].plot(mean3, '--', label=labels[2], linewidth= 3)
...
axes[0].legend(fontsize=20, loc=3)
...
if plot_error:
    ...
    axes[1].legend(fontsize=20, loc=3)
...
plt.tight_layout()
plt.savefig('tikzpicture_plots/fig2a.pdf', format="pdf", bbox_inches="tight")
